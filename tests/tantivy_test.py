...
class TestCollectU64FastField:
    """Tests for Searcher.collect_u64_fast_field().

    Uses a local index with an unsigned (u64) fast field so the type check
    passes.  add_integer_field maps to i64 in Tantivy, so the conftest
    ram_index_numeric_fields fixture is not suitable here.
    """
    ...
    def test_returns_values_in_hit_order(self, u64_index):
        """Values come back in the same order as doc_addresses."""
        searcher = u64_index.searcher()
        query = u64_index.parse_query("beta", ["body"])
        result = searcher.search(query, 10)
        addrs = [addr for _, addr in result.hits]
        ids = searcher.collect_u64_fast_field("doc_id", addrs)
    ...
    def test_all_docs_covered(self, u64_index):
        """All three documents are reachable via the fast field."""
        searcher = u64_index.searcher()
        query = u64_index.parse_query("gamma", ["body"])
        result = searcher.search(query, 10)
        addrs = [addr for _, addr in result.hits]
        ids = searcher.collect_u64_fast_field("doc_id", addrs)
        assert set(ids) == {101, 202, 303}

    def test_empty_input_returns_empty(self, u64_index):
        """Empty address list returns empty list without error."""
        searcher = u64_index.searcher()
        assert searcher.collect_u64_fast_field("doc_id", []) == []

    def test_single_match(self, u64_index):
        """Single result returns a one-element list."""
        searcher = u64_index.searcher()
        query = u64_index.parse_query("alpha", ["body"])
        result = searcher.search(query, 10)
        addrs = [addr for _, addr in result.hits]
        ids = searcher.collect_u64_fast_field("doc_id", addrs)
        assert ids == [101]

    def test_unknown_field_raises(self, u64_index):
        ...
        with pytest.raises(ValueError, match="Unknown field"):
            searcher.collect_u64_fast_field("nonexistent", addrs)

    def test_non_fast_field_raises(self, u64_index):
        ...
        with pytest.raises(ValueError, match="not a fast field"):
            searcher.collect_u64_fast_field("active", addrs)

    def test_wrong_type_field_raises(self, u64_index):
        """ValueError when the field is fast but is f64, not u64."""
        searcher = u64_index.searcher()
        query = u64_index.parse_query("gamma", ["body"])
        result = searcher.search(query, 1)
        addrs = [addr for _, addr in result.hits]
        with pytest.raises(ValueError, match="not a u64 field"):
            searcher.collect_u64_fast_field("score", addrs)
