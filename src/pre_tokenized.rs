...
pub(crate) struct Token {
    ...
    #[pyo3(get)]
    pub(crate) char_offset_from: usize,
    #[pyo3(get)]
    pub(crate) char_offset_to: usize,
}
...
impl Token {
    ...
    ///     char_offset_from (int): Offset (codepoint index) of the first character of the token.
    ///     char_offset_to (int, optional):
    ///         Offset (codepoint index) of the last character of the token + 1.
    ///         The text that generated the token should be obtained by text[char_offset_from:char_offset_to].
    ...
    #[pyo3(signature = (text, position, char_offset_from, char_offset_to=None))]
    fn new(
        text: &str,
        position: usize,
        char_offset_from: usize,
        char_offset_to: Option<usize>,
    ) -> Self {
        let char_offset_to = char_offset_to
            .unwrap_or_else(|| char_offset_from + text.chars().count());
        Self {
            text: text.to_owned(),
            position,
            char_offset_from,
    ...
    fn __repr__(&self) -> String {
        format!(
            "Token(text='{}', position={}, char_offset_from={}, char_offset_to={})",
            self.text, self.position, self.char_offset_from, self.char_offset_to
        )
    }
    ...
    fn __richcmp__(
        ...
        match op {
            pyo3::basic::CompareOp::Eq => (self.text == other.text
                ...
                && self.char_offset_from == other.char_offset_from
                && self.char_offset_to == other.char_offset_to)
                .into_py_any(py),
...
impl PreTokenizedString {
                ...
                tokens: tokens
                    ...
                    .map(|token| {
                        let offset_from = byte_offsets
                            .get(token.char_offset_from)
                            .copied()
                            .unwrap_or(text.len());
                        let offset_to = byte_offsets
                            .get(token.char_offset_to)
                            .copied()
                            .unwrap_or(text.len());
                        tvt::Token {
                            offset_from,
    ...
    fn tokens(&self) -> Vec<Token> {
        ...
        self.inner
            ...
            .iter()
            .map(|token| {
                let char_offset_from = byte_offsets
                    .binary_search(&token.offset_from)
                    .unwrap_or(character_count);
                let char_offset_to = byte_offsets
